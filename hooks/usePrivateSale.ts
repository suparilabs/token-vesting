...
function getVestingSchedulesCount(contract: Contract): (address: string) => Promise<Number> {
  return async (): Promise<Number> => contract.getVestingSchedulesCount().then((result: Number) => result);
}

export function usePrivateSale(address?: string | null, suspense = false): SWRResponse<Number> {
