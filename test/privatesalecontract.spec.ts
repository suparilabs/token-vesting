import { ethers, deployments, getNamedAccounts, artifacts } from "hardhat";
import { expect } from "chai";
import { Token } from "../src/types";
import { MockTokenVesting } from "../src/types/MockTokenVesting";
import { deployContract } from "ethereum-waffle";

describe("PrivateSaleContract", function () {
  before(async function () {
    const { admin, owner, alice, bob, charlie } = await getNamedAccounts();
    this.signers.admin = await ethers.getSigner(admin);
    this.signers.owner = await ethers.getSigner(owner);
    this.signers.alice = await ethers.getSigner(alice);
    this.signers.bob = await ethers.getSigner(bob);
    this.signers.charlie = await ethers.getSigner(charlie);
    await deployments.fixture("PrivateSaleContract");
    await deployments.fixture("Token");
    this.token = <Token>await ethers.getContract("Token");
    this.mockTokenVesting = <MockTokenVesting>await ethers.getContract("MockTokenVesting");
  });
  // beforeEach(async function () {
  //     const tokenArtifacts = await artifacts.readArtifact("Token")
  //     this.testToken = await deployContract()
  //     Token.deploy("Test Token", "TT", 1000000);
