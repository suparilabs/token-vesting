    ...
    const handleClick = async(e) => {
      if(mockData.length > 0 || mockData !== undefined){
        for(var i=0; i<mockData.length; i++) {
          beneficiary = mockData[i] ? "0x" + mockData[i].User_Address : "0";
          start = mockData[i] ? Date.parse(mockData[i].BlockTime_UTC) : 0;
          amount = mockData[i] ? parseInt(mockData[i].Tx_Amount + '000000000000000000') : 0;
